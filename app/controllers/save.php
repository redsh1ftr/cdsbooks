...
class InvoicesController extends \BaseController {
	...
	public function remakeInvoice(){
		...
		$npdf = PDF::loadView('refinalize', array(
			...
			$npdf->save("//CDS002/Scanned/Joe/invoices/$getoneinv->inv_number - $getoneinv->jobnum.pdf");
	...
	public function finalize(){
		...
		$npdf = PDF::loadView('finalize', array(
			...
			$npdf->save("//CDS002/Scanned/Joe/invoices/$newname - $name.pdf");

	
			$currentinvoice->update(array('invoice_number' => $newinvnum));
		...
		$nextatty = Input::get('pnum');
	 	 
		return View::make('invoiceform2', compact('nextatty'));
	...
	public function updateinvoice_address()
	{
		...
		foreach($addr as $add){
			...
			$add->addr1 = Input::get('addr1');
			$add->addr2 = Input::get('addr2');
			$add->addr3 = Input::get('addr3');
